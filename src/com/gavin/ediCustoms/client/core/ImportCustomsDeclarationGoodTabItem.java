...
public class ImportCustomsDeclarationGoodTabItem extends MyTabItem implements CustomsDeclarationPanelListener {
	...
	public ImportCustomsDeclarationGoodTabItem() {
		setText("进口料件");
		store = new ListStore<BeanModel>();
		store.sort("no", Style.SortDir.ASC);
		beanModelFactory = BeanModelLookup.get().getFactory(CustomsDeclarationGood.class);
	...
	protected void onRender(Element parent, int index) {
		...
		addButton.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				beanModel = beanModelFactory.createModel(new CustomsDeclarationGood());
				formPanel.reset();
				...
				formBindings.addListener(Events.Bind,
						new Listener<BindingEvent>() {
							...
							public void handleEvent(BindingEvent be) {
								...
								if (contractHead.getImportCurrency()!=null) {
									formBindings.getModel().set("currency", contractHead.getImportCurrency());
								}
	...
	public void refreshStores() {
		...
		getBusinessService().listCustomsDeclarationGood(selectedCustomsDeclarationHead.getId(), getCallback);
	...
	private FormPanel createForm() {
		...
		noInContract.setFieldLabel("料件序号");
		...
		originCountry.setFieldLabel("原产国家");
		...
		good.addListener(Events.SelectionChange, new Listener<SelectionChangedEvent<BeanModel>>() {
			...
			public void handleEvent(SelectionChangedEvent<BeanModel> be) {
				...
				if (selectedCustomsDeclarationHead.getContractHeadId() != null) {
					formBindings.getModel().set("contractGoodId", be.getSelectedItem().get("id"));
					formBindings.getModel().set("noInContract", be.getSelectedItem().get("no"));
					...
					formBindings.getModel().set("codeTS", be.getSelectedItem().get("code").toString()+be.getSelectedItem().get("plusCode").toString());
					formBindings.getModel().set("name", be.getSelectedItem().get("name"));
					formBindings.getModel().set("shenbaoguifan", be.getSelectedItem().get("goodModel"));
					formBindings.getModel().set("declarePrice", be.getSelectedItem().get("price"));				
					...
					formBindings.getModel().set("originCountry", be.getSelectedItem().get("originCountry"));
					formBindings.getModel().set("taxMode", be.getSelectedItem().get("taxMode"));
				} else {
					formBindings.getModel().set("code", be.getSelectedItem().get("code"));
					formBindings.getModel().set("plusCode", be.getSelectedItem().get("plusCode"));
					formBindings.getModel().set("codeTS", be.getSelectedItem().get("code").toString()+be.getSelectedItem().get("plusCode").toString());
					formBindings.getModel().set("name", be.getSelectedItem().get("name"));
					System.out.println(be.getSelectedItem().get("unit1"));
					formBindings.getModel().set("unit1", be.getSelectedItem().get("unit1"));
					formBindings.getModel().set("unit2", be.getSelectedItem().get("unit2"));
				}
			}
...
public Object convertFieldValue(Object value) {
 if (value == null) {return null;}
		...
		final AsyncCallback<Long> newCallback = new AsyncCallback<Long>() {
			...
			public void onSuccess(Long result) {
				if (result == 0) {
					...
					CustomsDeclarationGood newItem = (CustomsDeclarationGood) beanModel.getBean();
	...
	private void update() {
		...
		list.add((CustomsDeclarationGood) grid.getSelectionModel().getSelection().get(0)
				.getBean());
	...
	public void refreshGoodStore() {
		...
		if (selectedCustomsDeclarationHead.getContractHeadId() != null) {
			if (selectedCustomsDeclarationHead.getIsExport()) {
				getBusinessService().listContractProduct(selectedCustomsDeclarationHead.getContractHeadId(),contractProductCallback);
			} else {
				getBusinessService().listContractMaterial(selectedCustomsDeclarationHead.getContractHeadId(),contractMaterialAsyncCallback);
			}
				
		} else {
			getDictionaryService().listGoodClassification(goodClassificationCallback);
		}
