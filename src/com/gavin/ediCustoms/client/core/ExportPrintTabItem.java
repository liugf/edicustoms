...
public class ExportPrintTabItem extends MyTabItem implements CustomsDeclarationPanelListener{
	...
	private FormPanel createForm() {
		...
		enterpriseComboBox.addListener(Events.SelectionChange, new Listener<SelectionChangedEvent<BeanModel>>() {
			...
			public void handleEvent(SelectionChangedEvent<BeanModel> be) {
				...
				registeName_e.setValue((String)be.getSelectedItem().get("registeName"));
				address_e.setValue((String)be.getSelectedItem().get("address"));
				telephone_e.setValue((String)be.getSelectedItem().get("telephone"));
				fax_e.setValue((String)be.getSelectedItem().get("fax"));
			}
		});
		...
		final AsyncCallback<PayWay> payWayCallback = new AsyncCallback<PayWay>() {
			public void onFailure(Throwable caught) {
				MessageBox.alert("警告", "发生错误，请刷新！", null);
			}

			public void onSuccess(PayWay result) {
				payWay.setValue(result.getName());
			}
		};
		...
		customsDeclarationHeadComboBox.addListener(Events.SelectionChange, new Listener<SelectionChangedEvent<BeanModel>>() {
			...
			public void handleEvent(SelectionChangedEvent<BeanModel> be) {
				...
				getDictionaryService().getPort(be.getSelectedItem().get("destinationPort").toString(), destinationPortCallback);
				...
				getDictionaryService().getPayWay((String)be.getSelectedItem().get("payWay"), payWayCallback);
		...
		printButton2.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {				
				...
				for(Field field:formPanel.getFields()){
					...
					if (field.getName()!=null) {						
						...
						if (key.equals("declareTime")) {
							...
							paramsMap.put(key, value);
						}else if (key.equals("transportTool")) {
							String value=field.getRawValue()==null?"":field.getRawValue();	
							paramsMap.put(key, value);
						}else{
							String value=field.getValue()==null?"":field.getValue().toString();	
							paramsMap.put(key, value);
				...
				openNewWindow("/invoice2.do",paramsMap);	
			}
		});
		
		Button printButton3= new Button("装箱单",Resources.ICONS.printer());
	...
	public void refresh(){
		...
		if (formPanel!=null) {
			...
			//customsDeclarationHeadComboBox.fireEvent(Events.SelectionChange);
		}
