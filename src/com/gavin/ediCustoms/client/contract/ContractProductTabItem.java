...
public class ContractProductTabItem extends MyTabItem {
	...
	protected void onRender(Element parent, int index) {
		...
		addButton.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				beanModel = beanModelFactory.createModel(new ContractProduct());
				formPanel.reset();
	...
	private FormPanel createForm() {
		...
		voucher.addListener(Events.SelectionChange, new Listener<SelectionChangedEvent<BeanModel>>() {
			...
			public void handleEvent(SelectionChangedEvent<BeanModel> be) {
				...
				formBindings.getModel().set("goodClassificationId", be.getSelectedItem().get("goodClassificationId"));
		...
		NumberField processFee=new NumberField();		
		processFee.setPropertyEditorType(Double.class);
		processFee.setFormat(NumberFormat.getFormat("#0.00"));
		processFee.setAllowBlank(false);
		processFee.setName("processFee");
		processFee.setFieldLabel("工缴费");
		panel.add(processFee);
...
public Object convertFieldValue(Object value) {
 if (value == null) {return null;}
	...
	private void save() {
		final AsyncCallback<Long> newCallback = new AsyncCallback<Long>() {
			...
			public void onSuccess(Long result) {
				if (result == 0) {
					...
					ContractProduct newItem = (ContractProduct) beanModel.getBean();
		...
		getBusinessService().saveContractProduct(
				(ContractProduct) beanModel.getBean(), newCallback);
	}
	...
	private void update() {
		...
		list.add((ContractProduct) grid.getSelectionModel().getSelection().get(0)
				.getBean());
		getBusinessService().updateContractProduct(list, callback);
	}
	...
	private void delete() {
		...
		getBusinessService().deleteContractProduct(
	...
	private List<Long> models2Ids(List<BeanModel> models) {
		...
		for (Iterator<BeanModel> iterator = models.iterator(); iterator
				.hasNext();) {
			BeanModel beanModel = (BeanModel) iterator.next();
			ids.add(((ContractProduct) beanModel.getBean()).getId());
		}
		return ids;
	}
	...
	public void refreshVoucherStore() {
